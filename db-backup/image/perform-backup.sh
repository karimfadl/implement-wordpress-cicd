...
do
    ...
    if sqloutput=$(mysqldump -u $TARGET_DATABASE_USER -h $TARGET_DATABASE_HOST -p$TARGET_DATABASE_PASSWORD -P $TARGET_DATABASE_PORT $CURRENT_DATABASE 2>&1 > /tmp/$CURRENT_DATABASE-$(date "+%b-%d-%Y-%H-%M").sql)
    then
        ...
        if awsoutput=$(aws s3 cp /tmp/$CURRENT_DATABASE-$(date "+%b-%d-%Y-%H-%M").sql s3://$AWS_BUCKET_NAME$AWS_BUCKET_BACKUP_PATH/$CURRENT_DATABASE$(date "+%b-%d-%Y-%H-%M").sql 2>&1)
        then
